...
class BookingRepository extends BaseRepository
{
        ...
        if ($cuser) {
            ...
            if ($jobs) {
                foreach ($jobs as $jobitem) {
                    $jobCollection = ($jobitem->immediate == 'yes') ? $emergencyJobs : $normalJobs;
                    $jobCollection[] = $jobitem;
                }

                $noramlJobs = collect($normalJobs)->each(function ($item) use ($userId) {
    ...
    public function jobEnd($post_data = [])
    {
        ...
        $interval = $this->calculateTimeDifference($dueDate, $completedDate);
        ...
        $this->sendEmail('emails.session-ended', $email, $name, $subject, $data);
    ...
    private function calculateTimeDifference($dueDate, $completedDate)
    {
    ...
    {
        $sessionTime = explode(':', $interval);
        $hours = (int)$sessionTime[0];
        $minutes = (int)$sessionTime[1];
        return $hours . ' tim ' . $minutes . ' min';
    }

    private function sendEmail($view, $email, $name, $subject, $data)
    {
    ...
    public function sendSMSNotificationToTranslator($job)
    {
        ...
        if ($job->customer_physical_type == 'yes' && $job->customer_phone_type == 'no') {
            ...
            // It's a phone job or both (treated as phone job)
    ...
    public function getPotentialTranslators(Job $job)
    {
        ...
        $users = User::getPotentialUsers($translator_type, $joblanguage, $gender, $translator_level, $blacklistedTranslators);
    ...
    private function changePendingStatus($job, $data, $changedTranslator)
    {
        ...
        if ($data['admin_comments'] === '' && $status === 'timedout') {
            return false;
        }
    ...
    private function changeAssignedStatus($job, $data)
    {
        ...
        if (in_array($status, ['withdrawbefore24', 'withdrawafter24', 'timedout'])) {
            ...
            if ($status == 'timedout' && $data['admin_comments'] == '') {
                return false;
            }
            ...
            if (in_array($status, ['withdrawbefore24', 'withdrawafter24'])) {
                ...
                $this->mailer->send($customerEmail, $user->name, 'Information om avslutad tolkning för bokningsnummer #' . $job->id, 'emails.status-changed-from-pending-or-assigned-customer', $dataEmail);

                $this->mailer->send($translatorEmail, $translator->user->name, 'Information om avslutad tolkning för bokningsnummer # ' . $job->id, 'emails.job-cancel-translator', $dataEmail);
            }
    ...
    private function changeTranslator($current_translator, $data, $job)
    {
        ...
        if (!is_null($current_translator) || (isset($data['translator']) && $data['translator'] != 0) || $data['translator_email'] != '') {
            if (!is_null($current_translator) && ((isset($data['translator']) && $current_translator->user_id != $data['translator']) || $data['translator_email'] != '') && (isset($data['translator']) && $data['translator'] != 0)) {
                ...
                $new_translator_data['user_id'] = $data['translator'];
    ...
    public function acceptJobWithId($jobId, $cuser)
    {
        ...
        if (!Job::isTranslatorAlreadyBooked($jobId, $cuser->id, $job->due)) {
            if ($job->status == 'pending' && Job::insertTranslatorJobRel($cuser->id, $jobId)) {
                ...
                if ($this->isNeedToSendPush($user->id)) {
                    $users_array = [$user];
                    $this->sendPushNotificationToSpecificUsers($users_array, $jobId, $notificationData, $msg_text, $this->isNeedToDelayPush($user->id));
                }
        ...
        if ($cuser->is('customer')) {
            ...
            if ($translator) {
                ...
                if ($this->isNeedToSendPush($translator->id)) {
                    ...
                    $this->sendPushNotificationToSpecificUsers($users_array, $jobId, $notificationData, $msg_text, $this->isNeedToDelayPush($translator->id));
                }
            }
        } else {
            if ($job->due->diffInHours(Carbon::now()) > 24) {
                ...
                if ($customer) {
                    ...
                    if ($this->isNeedToSendPush($customer->id)) {
                        ...
                        $this->sendPushNotificationToSpecificUsers($users_array, $jobId, $notificationData, $msg_text, $this->isNeedToDelayPush($customer->id));
                    }
                }
